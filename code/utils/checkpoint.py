...
def init_checkpoint(conf):
    ...
    while invalid:
        ...
        conf.time_stamp_ = (
            f"{time_id}_task-{conf.task}_"
            f"inlr-{conf.inner_lr:.1E}_outlr-{conf.outer_lr:.1E}_"
            f"fas-{conf.fas}_"
            f"neps-{conf.nepisodes}_epsepcs-{conf.episode_epochs}_"
            f"evalepsbt-{conf.eval_every_episode_batch}_"
            f"nproc-{conf.nproc_per_node if hasattr(conf,'nproc_per_node') else conf.task_batch_size}_"
            f"nshots-{conf.num_shots}_"
            f"reinit-{conf.reinit if hasattr(conf,'reinit') else 'False'}_"
            f"sd-{conf.manual_seed}"
        )
    ...
    assert len(os.path.abspath(conf.checkpoint_root)) < 255, len(
        os.path.abspath(conf.checkpoint_root)
    )
